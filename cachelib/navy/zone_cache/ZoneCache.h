...
class ZoneCache final : public Engine {
 public:
  ...
  explicit ZoneCache(Config&& config);
  ZoneCache(const ZoneCache&) = delete;
  ZoneCache& operator=(const ZoneCache&) = delete;
  ~ZoneCache() override = default;
  ...
  // Inserts a key-value pair into ZoneCache.
  //
  ...
  // Flushes all buffered (in flight) operations in ZoneCache.
  void flush() override;

  // Resets ZoneCache to its initial state.
  void reset() override;

  // Serializes ZoneCache state to a RecordWriter.
  //
  // @param rw   RecordWriter to serialize state
  void persist(RecordWriter& rw) override;

  // Deserialize ZoneCache state from a RecordReader.
  //
  ...
  // Exports ZoneCache stats via CounterVisitor.
  //
  // @param visitor   CounterVisitor to export stats
  void getCounters(const CounterVisitor& visitor) const override;

  // Gets the maximum item size that can be inserted into ZoneCache.
 ...
 private:
  ...
  ZoneCache(Config&& config, ValidConfigTag);
  ...
  bool isValidRecoveryData(const serialization::ZoneCacheConfig& config) const;

  static serialization::ZoneCacheConfig serializeConfig(const Config& config);
  ...
  const serialization::ZoneCacheConfig config_;
  ...
  mutable util::PercentileStats debugLatencyEstimator_;
};
} // namespace navy
} // namespace cachelib
} // namespace facebook
